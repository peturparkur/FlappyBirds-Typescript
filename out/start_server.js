...
class Client {
    constructor(identifier, connection) {
        this.id = identifier;
...
class Room {
    constructor(identifier, clients, gameId) {
        this.clients = Array(); //we assume it's in the same order as birds
        this.id = identifier;
...
const rooms = new Map();
const clientMap = new Map(); //maps clientId -> roomId
class GameInstance {
...
const games = new Map(); //identifiability of games
...
function Broadcast(data, exclude = null) {
    for (let client of clients) {
        let clientConnection = client[1].connection;
        //send to same person?
...
//broadcast to every client data
function BroadcastOne(data, exclude) {
    for (let client of clients) {
        let clientConnection = client[1].connection;
        //send to same person?
    ...
    */
    connection.on("message", function (message) {
        ...
        if (msgData.Method === "JoinRoom") {
            let roomId = msgData.payload.roomId;
            let clientId = msgData.payload.clientId;
        ...
        if (msgData.Method === "CreateGame") {
            let host = msgData.ClientId;
            let numPlayers = msgData.numPlayers;
            let width = msgData.width;
            let height = msgData.height;
            let fwSpeed = msgData.fwSpeed;
            ...
            let roomId = guid();
            let room = new Room(roomId, new Array(), gameId); //create a room, add the first client to be the creator
        ...
        if (msgData.Method === "GameAction") {
            let cltId = msgData.payload.clientId;
            let action = msgData.payload.action;
