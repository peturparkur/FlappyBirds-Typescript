...
//draw's circle
function DrawCircle(ctx, circle = new Circle(), color = "black", outline = false) {
...
function DrawRect(ctx, rect = new Rect(), color = "black", outline = false) {
    ctx.beginPath();
...
function Intersect(circle, rect) {
    ...
    if (distance.x > rect.width * 0.5 + circle.radius)
        return false;
    if (distance.y > rect.height * 0.5 + circle.radius)
        return false;
    //if distance from center is less than 0.5*width then it's definite collision
    if (distance.x <= rect.width * 0.5)
        return true;
    if (distance.y <= rect.height * 0.5)
        return true;
    ...
    if (d.squareMagnitude <= circle.radius * circle.radius)
        return true;
    return false;
}
